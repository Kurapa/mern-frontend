...
export default function FaultyLogin() {
    ...
    const arr={
      ...
      function handlesubmit(event){
          ...
          Axios.post("https://mern-backend-server-kurapa.onrender.com/faculty-route/login",arr)
          .then((res)=>{
              if(res.data.msg==='fail'){
                  ...
                  console.log(res.data)
                   navigate(`/faculty-home/${res.data.id}/${res.data.name}/${res.data.regno}`)
                  }
          })
          .catch(
              (err)=>{
      ...
      <div className='col-10 col-lg-6 mt-4 pt-4 d-flex align-items-lg-center align-items-start justify-content-center  shadow rounded-5' >
        <div>
            <form onSubmit={handlesubmit}>
                ...
                <button type="submit" className="btn btn-primary col-lg-8 col-10"  id='PortalLogIn'>Log in</button>
                </div>
            </form>
            </div>
            </div>
